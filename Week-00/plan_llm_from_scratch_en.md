...
## TL;DR
- **Achievable in ~3.5–4 months part-time (10–15 hrs/week)**, with the material spread evenly across 15 weeks so no single week is overloaded: Weeks 1–7 build and pretrain a GPT-2-class model from scratch (Raschka's book + Karpathy + Giles Thomas; instruction fine-tuning and alignment are now two separate weeks); Weeks 8–11 do applied RAG and QLoRA/MLX fine-tuning; Weeks 12–15 build an agentic SDLC assistant — including a dedicated **Graph Engineering** week (knowledge graph as shared memory for multi-agent systems, per the documents in `docs/`). The hard constraint is your 8GB RTX 3070 Ti — it is excellent for learning-scale from-scratch coding and 7B–8B QLoRA fine-tuning, but full GPT-2 pretraining and any 13B+ work should go to cheap cloud GPUs.
...
**The three primary sources you identified are current, well-maintained, and form a coherent spine.** Raschka's *Build a Large Language Model (From Scratch)* (Manning, 2024; repo `github.com/rasbt/LLMs-from-scratch`) is the core text. Its sequel *Build a Reasoning Model (From Scratch)* is in Early Access now and scheduled for full Manning release on July 28, 2026 — Raschka announced on LinkedIn that "all chapters" are complete and the book is "Now in Early Access," and the **code repo `github.com/rasbt/reasoning-from-scratch` is already public** and usable for GRPO/RLVR and inference-scaling work. Giles Thomas's blog series reached part 33 (April 2026), and critically he documents training a GPT-2-small-class model on his own RTX 3090 in "just over 48 hours on my own hardware" (achieving loss 3.944 vs. the original GPT-2's 3.500) — a near-perfect reference for your hardware questions. The FareedKhan-dev repo now extends beyond pretraining to a full from-scratch alignment suite (Base → SFT → Reward Model → PPO/DPO → GRPO) in pure PyTorch on real datasets (Alpaca, Dolly, Anthropic HH-RLHF, UltraFeedback, GSM8K).

**Karpathy's ecosystem evolved in a way that directly helps you.** His "Neural Networks: Zero to Hero" series remains the best free from-scratch video course (micrograd → makemore → nanoGPT). In October 2025 he released **nanochat** (`github.com/karpathy/nanochat`), a ~8,000-line full-stack ChatGPT-clone pipeline (tokenizer → pretrain → midtrain → SFT → GRPO → web UI) — the intended capstone of his still-in-development LLM101n course. As of mid-2026 Karpathy joined Anthropic. Per the nanochat repo, "you can train your own GPT-2 capability LLM (which cost ~$43,000 to train in 2019) for only $48 (~2 hours of 8×H100 GPU node)"; the full `speedrun.sh` takes ~3 hours and "on a spot instance, the total cost can be closer to ~$15" (the 8×H100 node "is costing us about ~$24/hr"). So for you nanochat is primarily a *reading/forking* reference and an optional cloud capstone, not local work.

**On your 8GB RTX 3070 Ti, here is what is real:** QLoRA 4-bit fine-tuning of 7B (≈5GB) and 8B (≈6GB) models fits comfortably per Unsloth's official VRAM table; up to ~11B (7.5GB) is at the edge; 14B (8.5GB) just exceeds 8GB. Practical config: batch size 1–2, sequence length ≤1024, gradient checkpointing on. Pretraining a GPT-2 small (124M) from scratch is technically possible locally but slow — Giles needed ~48h for a comparable model on a 24GB 3090 that fit only batch=6; an 8GB card fits ~batch 1–2 with heavy gradient accumulation, pushing well past 48h — so do the pretraining run in the cloud.

**Cloud is cheap for the one-time pretrain.** Per RunPod's official RTX 4090 page (rechecked May 24, 2026), Community Cloud is from $0.34/hr; ToolChase's June 2026 RunPod review lists Secure Cloud "RTX 4090 from $0.69/hr, A100 from $1.49/hr, and H100 from $2.89/hr." Karpathy reproduced GPT-2 124M (12-layer, 10B FineWeb tokens, seq len 1024) on "one 8×A100 80GB SXM node [in] ~90 minutes…on Lambda this node goes for ~$14/hr, so the total cost…is about $20" (`karpathy/llm.c` Discussion #481). Giles trained his 163M model on 8×A100 40GB (Lambda) "in less than four hours…for about US$35, excluding validation."
...
- **Explain the math:** Paste an equation or paragraph from Raschka (e.g., scaled dot-product attention, cross-entropy, KL divergence in DPO) and ask Claude to derive it step by step, then to quiz you. Use it as a Socratic tutor: "Ask me three questions to check I understand causal masking."
- **Review your from-scratch code:** After you implement an attention block or training loop *yourself*, paste it and ask Claude to compare against the canonical Raschka/Karpathy implementation, flag bugs, and explain divergence. Do not let it write the first draft — implement first, review second. (Note: Karpathy himself reported that coding agents struggled on nanochat because the repo is "too far off the data distribution"; expect Claude to be strongest on standard PyTorch patterns and weaker on novel from-scratch tricks.)
...
- **Generate practice exercises and spaced repetition:** Ask Claude to produce exercises mirroring Raschka's "Test Yourself" guide, or flashcards on terms (RoPE, GQA, MoE, MLA).
...
- **Vietnamese ↔ English:** Raschka's liveBook AI assistant answers in any language; you can also have Claude explain a dense English passage in Vietnamese, then switch back to English technical terms.
...
- *Sources:* 3Blue1Brown "Neural Networks" and "Linear Algebra" visual series (3blue1brown.com); Raschka's free **"PyTorch in One Hour: From Tensors to Training Neural Networks on Multiple GPUs"** (sebastianraschka.com/teaching/pytorch-1h/); Raschka book Appendix A.
...
- *Sources:* Karpathy "Neural Networks: Zero to Hero" Lecture 1 (micrograd) and Lectures 2–4 (makemore); 3Blue1Brown "Attention in transformers, step-by-step" (Ch.6) and the MLP chapter; Jay Alammar's "The Illustrated Transformer" (jalammar.github.io/illustrated-transformer/); the original "Attention Is All You Need."
- *Task:* Follow Karpathy and build micrograd yourself; start makemore (bigram → MLP).
...
**Week 3 — Tokenization, embeddings, attention from scratch (Raschka ch.2–3).**
- *Objectives:* Implement BPE/data loading, token + positional embeddings, and self-attention → causal → multi-head, by hand.
- *Sources:* Raschka book ch.2 (Working with text data) and ch.3 (Coding attention mechanisms); companion reading: Giles Thomas parts 1–14 (especially part 8 trainable self-attention, part 9 causal attention, part 12 multi-head, part 13 "attention heads are dumb"); The Annotated Transformer (Harvard NLP, nlp.seas.harvard.edu).
- *Task:* Code the full attention stack from ch.3 yourself; verify shapes against the book repo.
- *Hardware:* 3070 Ti.
...
**Week 4 — Assemble and run the GPT model (Raschka ch.4).**
...
- *Sources:* Raschka ch.4 (Implementing a GPT model from scratch); Giles Thomas parts 15–19 (layer norm, feed-forward, residuals, wrapping up ch.4); Karpathy nanoGPT (`github.com/karpathy/nanoGPT`) as a cross-reference.
- *Task:* Instantiate the 124M config, load OpenAI GPT-2 pretrained weights (as the book shows) to confirm your architecture is correct, and generate text.
...
**Week 5 — Pretraining: the training loop + a real (cloud) GPT-2 run (Raschka ch.5).**
...
- *Sources:* Raschka ch.5 (Pretraining on unlabeled data) + Appendix D (bells & whistles); Giles Thomas parts 20–29 (training, cross-entropy, the "Interventions" mini-series on gradient clipping, dropout, LR, weight decay, weight tying, mixed precision — parts 32a–32m); Karpathy nanoGPT/llm.c "Reproduce GPT-2 124M" discussion; HF Ultra-Scale Playbook (huggingface.co/spaces/nanotron/ultrascale-playbook) for gradient-accumulation/parallelism concepts.
- *Task:* First, train on the small "The Verdict" dataset locally to validate your loop on the 3070 Ti. Then do a **real GPT-2-small pretraining run in the cloud** on FineWeb/FineWeb-Edu.
- *Hardware:* **Local 3070 Ti** for loop validation and a tiny model; **cloud** for the real run — rent a single RTX 4090 (RunPod from $0.34/hr Community) for a multi-hour run, or an 8×A100 node (Lambda, ~$14/hr for the node) to reproduce in <4 hrs for ~$35 (Giles's documented cost). On 8GB locally you'd use micro-batch 1–2, seq len 1024, gradient accumulation ~16–64 to hit ~0.5M-token effective batches (Karpathy's GPT-2 setup targets ~524,288 tokens/update).
- *Deliverable:* A pretrained small base-model checkpoint + a short write-up comparing your loss curve to the original GPT-2, modeled on Giles's analysis.
- *Time:* ~12–15 hrs (plus unattended training time).

**Week 6 — Instruction fine-tuning (Raschka ch.6–7 + LoRA).**
- *Objectives:* Fine-tune for classification (ch.6) and instruction-following (ch.7); apply LoRA and compare it against full fine-tuning.
- *Sources:* Raschka ch.6–7 + Appendix E (LoRA).
- *Task:* Fine-tune a classifier per ch.6; instruction-fine-tune your model (or a small pretrained one) following ch.7 with an Alpaca-style template; compare full FT vs. LoRA.
...
**Week 7 — Intro to alignment: SFT → Reward Model → DPO/PPO → GRPO (FareedKhan + reasoning repo).**
...
- *Sources:* FareedKhan-dev/train-llm-from-scratch `src/post_training/` (SFT/RM/PPO/DPO/GRPO in pure PyTorch on Alpaca, Dolly, Anthropic HH-RLHF, UltraFeedback, GSM8K); Raschka's `reasoning-from-scratch` repo for GRPO/RLVR and evaluation.
...
> **If time is tight in Phase 1:** the deepest-value weeks are 2–5 (backprop, attention, GPT assembly, pretraining). You can compress Week 7 alignment to *conceptual* understanding + one DPO run, and defer reasoning-model/GRPO depth to after the roadmap (the book ships fully July 2026 anyway).
...
- *Sources:* DeepLearning.AI "Retrieval Augmented Generation" course (free to audit) and/or freeCodeCamp "RAG from Scratch"; LlamaIndex and LangChain docs; NirDiamant/RAG_Techniques and sosanzma/rag-techniques-handbook on GitHub.
...
- *Sources:* RAGAS docs (context precision/recall, faithfulness, answer relevancy); LangSmith and Langfuse for tracing/LLM-as-judge; Cohere Rerank or open BGE cross-encoder reranker; GraphRAG (Microsoft) as optional advanced reading.
...
> **Where time is tight / what to defer:** Phase 1 alignment depth (full PPO/GRPO from scratch, Week 7) and Raschka's reasoning book (full release July 2026) are the most deferrable. The advanced-RAG week (11) and capstone (15) can each spill a few days. If you fall behind, protect Weeks 2–5 (the irreplaceable from-scratch core) and Weeks 12–15 (your actual goal), and trim Week 9 (Mac/MLX) to inference-only.
...
2. **Do the GPT-2 pretraining run in the cloud, not locally.** Budget ~$15–35 one-time on RunPod (single RTX 4090 from $0.34/hr) or Lambda (8×A100, node ~$14/hr → ~$35 for a <4-hr run, per Giles Thomas; ~$20 for Karpathy's 90-min llm.c run). Use the 3070 Ti only to validate your loop on a tiny dataset first. *Trigger to go cloud:* the moment your local run projects beyond ~24h.
...
## Caveats
- **Hardware time estimates for the 3070 Ti are extrapolations**, not measured benchmarks: they're inferred from Karpathy's general "4–24h single GPU" range (which assumes ≥24GB cards) and Giles Thomas's RTX 3090 (24GB, batch 6, ~48h) results. Your 8GB card with batch 1–2 + heavy gradient accumulation will be slower for the same token budget. Re-verify with a short local smoke test before committing to a long run.
...
- **Raschka's reasoning book** is in Early Access but not fully published until July 28, 2026; rely on the public `reasoning-from-scratch` repo for now.
