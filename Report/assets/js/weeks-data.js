...
window.WEEKS_DATA = [
 ...
 },
 {
  ...
  check:["ch.6: chuẩn bị dataset classification (spam) + sửa head","ch.6: fine-tune classifier, đo accuracy train/val/test","ch.7: format instruction dataset (Alpaca-style)","ch.7: instruction fine-tune + sinh phản hồi","Áp dụng LoRA (Appendix E) — so full FT vs LoRA","Chat thử với mini-model → ghi ví dụ"]
 },
 {
  ...
  check:["Vẽ lại pipeline: Pretrain → (Midtrain) → SFT → RM → PPO/DPO → GRPO/RLVR","Đọc FareedKhan src/post_training — hiểu SFT/RM/DPO","Hiểu loss Reward Model (log-sigmoid hiệu score)","Hiểu vì sao DPO bỏ được RM riêng + dạng loss DPO","Hiểu GRPO group-relative + vì sao hợp RLVR","Chạy MỘT stage alignment (SFT hoặc DPO) scaled-down","Viết alignment_notes.md: SFT vs DPO vs GRPO","So phản hồi trước/sau stage đã chạy → ghi ví dụ"]
 },
 {
  ...
  deliver:"Adapter 7B/8B đã fine-tune + <b>eval so base vs fine-tuned</b> trên held-out (<code>eval_notes.md</code>).",
  ...
  check:["Cài Unsloth + dependencies (CUDA khớp)","Chọn base model (Llama 3.1 8B / Qwen2.5 7B) 4-bit","Chuẩn bị dataset 500–1,000 mẫu (domain Finance Banking)","Cấu hình LoRA (r=16, α=16, target all proj) + SFTTrainer","Smoke test vài step → không OOM, loss giảm","Chạy full run + lưu adapter","Merge adapter + export GGUF (cho Tuần 9)","Eval base vs fine-tuned trên held-out → eval_notes.md"]
 },
 {
  ...
  deliver:"Local inference stack hoạt động + model MLX fine-tune + ghi chú <b>Mac vs 3070 Ti vs cloud</b> (<code>hardware_decision.md</code>).",
  ...
  check:["Cài mlx-lm (pip install mlx-lm) trên Mac","Tải model MLX-format (HF mlx-community/...)","LoRA fine-tune: mlx_lm.lora --train --iters 500","Fuse adapter: mlx_lm.fuse","Cài Ollama + Modelfile cho GGUF (Tuần 8) / MLX","Cài LM Studio, load model, test chat","So tốc độ Mac vs 3070 Ti cùng prompt","Viết hardware_decision.md"]
 },
 {
 ...
 },
 {
  ...
  deliver:"Single agent + kết nối MCP hoạt động + reflective loop chạy được + sơ đồ kiến trúc CornAgents.AI 1 trang (<code>cornagents_architecture.md</code>).",
  ...
  check:["Xem docs/5-layers-multi-agent.jpg — tự vẽ lại 5 tầng bằng lời mình","Đọc Claude Agent SDK docs — agent loop + tool use","Đọc MCP docs — server/client, transport","Build single agent: đọc repo → chạy 1 tool → structured output","Kết nối 1 MCP server (filesystem hoặc GitHub)","Build reflective loop: gen → eval → revise → stop rule","Hiểu 4 điều kiện của ratchet loop (verifiable/reversible/short/bounded)","So sánh LangGraph vs CrewAI cho nhu cầu của bạn","Chọn stack + lý do (regulated → LangGraph)","Vẽ cornagents_architecture.md (sơ đồ + tool boundaries + HITL)"]
 },
 {
  ...
  deliver:"Workflow multi-agent: requirement → <b>stories + design note + tests</b>, có human gate (<code>agent_design.md</code>).",
  ...
  check:["Đọc mục IV + VIII Karpathy-Loop PDF — 5 patterns + 6 câu hỏi","Map từng stage SDLC ↔ loại agent + pattern + I/O rõ ràng","Agent 1 — Requirements Analyst: request → stories + AC (RAG)","Agent 2 — Code Review: đọc diff/PR → criterion-level defects","Agent 3 — Test-Gen: từ story/code → test case","Định nghĩa artifact contract (schema) cho từng handoff","Nối thành graph (LangGraph state / CrewAI crew)","Thêm human approval gate giữa các stage","Scope tool least-privilege cho từng agent","Chạy thử 1 requirement Finance Banking end-to-end","Ghi agent_design.md"]
 },
 {
  ...
  deliver:"<code>kg_pipeline.py</code> chạy trên corpus của bạn + so sánh grounded vs ungrounded + precision/recall vs gold set (<code>graph_notes.md</code>).",
 ...
 },
 {
  ...
  deliver:"Capstone CornAgents.AI demo được + báo cáo evaluation (<code>eval_rubric.md</code>) + retrospective nối về Phase 1 (<code>retrospective.md</code>).",
  ...
  check:["Chốt 1 use case Finance Banking (spec-to-stories + review, giữ tổng quát)","Ghép RAG (Tuần 10–11) + agents (Tuần 13) + KG (Tuần 14) thành 1 luồng","(Tùy chọn) cắm model fine-tuned (Tuần 8/9) cho sub-task","Khai báo complexity budget trước khi chạy","Instrument tracing (Langfuse/LangSmith)","Viết eval rubric → eval_rubric.md","Đo: success rate, human-override rate, groundedness","Kiểm tra 'every output can be traced...' với demo","Demo end-to-end (script / video ngắn)","Viết retrospective.md (nối về Phase 1)"]
 }
];
